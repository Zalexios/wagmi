...
`useWaitForTransaction` will now return an `error` (and invoke the `onError` callback) if the transaction has reverted.
...
See the wagmi [TypeScript docs](/docs/typescript) for more information on TypeScript support in this version.
